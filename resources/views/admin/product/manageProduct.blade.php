...
<table class="table table-hover table-bordered">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td><img src="{{ asset('uploads/product_photo/') }}/{{ $product->productPicture }}" width="100"></td>
        <td>{{ $product->publicationStatus ==1 ? 'Published' : 'Unpublished' }}</td>
        <th><a href="{{ url('/view_product/'.$product->id) }}">View </a>|<a href="{{ url('/edit_product/'.$product->id) }}"> Edit </a>|<a href="{{ url('/delete_product/'.$product->id) }}" onclick="return confirm('Are you confirm to delete?')">Delete</a></th>
      </tr>
      @endforeach
    </tbody>
  </table>
